...
public extension Array where Element: Differentiable {
    public func differences(from old: Array<Element>) -> Differences.Result {
        return Differences.differences(from: old, to: self)
    }

    public func differences(to new: Array<Element>) -> Differences.Result {
        return Differences.differences(from: self, to: new)
    }
}

private func differences<A: Differentiable>(from old: Array<A>, to new: Array<A>) -> Differences.Result {
    ...
    func records(
        ...
        return array.enumerated().map { (index, element) -> Record in
